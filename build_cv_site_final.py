...
def build(lang):
    ...
    c.linkURL('https://www.linkedin.com/in/ana-noronha/', (459.8, rl(54.9, TS), 459.8 + _lw, rl(54.9, TS) + TS))
    ...
    if is_en:
        ...
        p2y = wrap('· northgamefishing.com — international service website | 2023 – Present', False, main_x, p2y, MID, main_w, 13.5)
...
def postprocess_pdf(fn, is_en):
    """Add metadata, bookmarks and semantic tagged-PDF structure (pypdf only, no fitz)."""
    ...
    if is_en:
        ...
        toc = [
            ('Experiência Profissional', 0),
            ('Experiência Adicional', 1),
            ('Educação', 1),
            ('Projetos Digitais', 1),
            ('Competências', 1),
            ('Ferramentas Digitais', 1),
            ('Idiomas', 1),
        ]
